...
static inline TatrResult tatr(const char *args)
{
    ...
    while (open_pipes > 0) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            // POLLHUP (or n==0 above fell through): pipe closed.
            if (pfds[i].revents & (POLLHUP | POLLERR | POLLNVAL)) {
                // Drain any remaining bytes before marking closed.
                ssize_t n;
                String_Builder *sb = (i == 0) ? res.out : res.err;
...
static inline bool tf_create_raw_issue(const char *id, const char *title, const char *status, const char *priority, const char *tags, const char *body)
{
