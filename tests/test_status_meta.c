...
{
    Tatr_Result r = tatr("status --limit 1");
    ASSERT_CMD_OK(r);
}

static void test_status_custom_stale_days(void)
{
    Tatr_Result r = tatr("status --stale-days 999");
    ASSERT_CMD_OK(r);
...
{
    Tatr_Result r = tatr("help frobnicate");
    ASSERT_CMD_FAIL(r);
